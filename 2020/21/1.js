...
function solution(data){
	...
	console.log(Object.keys(allergenList).sort((a,b)=>allergenList[a].length-allergenList[b].length))
	...
	console.log(finalAllergenList, allergenList);
	const ingredientsWithAllergens = Object.keys(finalAllergenList).map(key=>finalAllergenList[key]);
	return Object.keys(ingredientsOccurances)
		.reduce(
			(total, ingredientName)=>total += ingredientsWithAllergens.indexOf(ingredientName) < 0 ? ingredientsOccurances[ingredientName] : 0, 0 );
}
