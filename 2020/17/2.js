...
class Point{
	...
	isNeighbourOf(point){
		...
		&& point.w >= this.w-1 && point.w <= this.w+1;
	}
	isEqual(point){
			...
			for(let z=this.z-1;z<=this.z+1;z++){
				for(let y=this.y-1;y<=this.y+1;y++){
					for(let x=this.x-1;x<=this.x+1;x++){
						if( !(w===this.w && z===this.z && y===this.y && x===this.x) ){
							neighbours.push(new Point(x, y, z, w, false) );
						}
					}
				}
			}
...
function solution(data){
	...
	console.log(data);
	let state = [...data.map((line,yIndex)=>line.map((val,xIndex)=>new Point(xIndex, yIndex, 0, 0, val==='#'))).flat()];
	let cycle = 0;
	while(cycle<6){
...
function processState(state){
	// state is an array of active points.
	...
	const nextState = nextStateSpace.map(point=>{
		...
		return new Point(point.x, point.y, point.z, point.w, isActive);
...
function displayState(state){
	...
	let xMin=Infinity, xMax=-Infinity, yMin=Infinity, yMax=-Infinity, zMin=Infinity, zMax=-Infinity, wMin=Infinity, wMax=-Infinity;
