...
function getMagnitudeOfSum(sumValue){
	let currentValue = sumValue;
	let done = false;
	let nextValue = '';
	while(!done){
	...
	return getMagnitude(currentValue);
}
function solution(data){
	let hightestMagnitude = Number.NEGATIVE_INFINITY;
	for( let i=0; i<data.length-1;i++){
		for(let j=i+1;j<data.length; j++){
			const a = data[i];
			const b = data[j];
			const abMagnitude = parseInt(getMagnitudeOfSum(`[${a},${b}]`));
			if( abMagnitude > hightestMagnitude ){
				hightestMagnitude = abMagnitude;
			}
			const baMagnitude = parseInt(getMagnitudeOfSum(`[${b},${a}]`));
			if( baMagnitude > hightestMagnitude ){
				hightestMagnitude = baMagnitude;
			}
		}
	}
	return hightestMagnitude;
}
